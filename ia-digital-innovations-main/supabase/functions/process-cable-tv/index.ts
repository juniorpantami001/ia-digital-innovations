...
serve(async (req) => {
  ...
  try {
    ...
    const { provider, smartCardNumber, planName, amount } = await req.json();

    console.log('Processing cable TV purchase:', { provider, smartCardNumber, planName, amount, userId: user.id });
    ...
    console.log('Cable TV purchase completed:', { reference, status: finalStatus });
    ...
    console.error('Error in process-cable-tv:', error);
    return new Response(
